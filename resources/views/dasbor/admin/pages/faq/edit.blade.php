...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                {!! Form::model($data,array('url'=>'app/faq/update/'.$data->id))!!}
                @csrf
                @method('put')
                <div class="mb-3">
                    <label for="product-name" class="form-label">Pertanyaan <span class="text-danger">*</span></label>
                    {!! Form::text('pertanyaan',null,['required','id'=>'pertanyaan','class'=>'form-control','placeholder'=>'Judul Halaman'])!!}
                </div>

                <div class="mb-3">
                    <label for="product-konten" class="form-label">Jawaban <span class="text-danger">*</span></label>
                    <textarea name="jawaban" class="ckeditor form-control" id="jawaban" value="{{ old('jawaban',$data->jawaban) }}"
                        cols="30" rows="10">{{ old('jawaban',$data->jawaban) }}</textarea>
                </div>
